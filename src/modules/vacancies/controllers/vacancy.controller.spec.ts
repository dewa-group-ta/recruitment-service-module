...
describe("VacancyController", () => {
  ...
  describe("create", () => {
    it("should create a new vacancy", async () => {
      ...
      expect(vacancyService.create).toHaveBeenCalledWith(createVacancyDto, "user-1");
      expect(result).toEqual(mockVacancyResponse);
    });

    it("should throw BadRequestException when creation fails", async () => {
      const createVacancyDto: CreateVacancyDto = {} as CreateVacancyDto;
      vacancyService.create.mockRejectedValue(new BadRequestException("Invalid data"));

      await expect(controller.create(createVacancyDto, mockRequest)).rejects.toThrow(
        BadRequestException
      );
    });
  });
  ...
  describe("update", () => {
    it("should update vacancy successfully", async () => {
      const updateVacancyDto: UpdateVacancyDto = { title: "Updated Software Engineer" };
      vacancyService.update.mockResolvedValue(mockVacancyResponse);

      const result = await controller.update("vacancy-1", updateVacancyDto, mockRequest);

      expect(vacancyService.update).toHaveBeenCalledWith("vacancy-1", updateVacancyDto, "user-1");
      expect(result).toEqual(mockVacancyResponse);
    });

    it("should throw NotFoundException when vacancy not found", async () => {
      const updateVacancyDto: UpdateVacancyDto = {};
      vacancyService.update.mockRejectedValue(new NotFoundException("Vacancy not found"));
  ...
  describe("findOne", () => {
    ...
    it("should throw NotFoundException when vacancy not found", async () => {
      vacancyService.findOne.mockRejectedValue(new NotFoundException("Vacancy not found"));
  ...
  describe("findAll", () => {
    it("should return paginated vacancies with filters", async () => {
      const mockPaginatedResponse = {
        data: [{ ...mockVacancyResponse, totalApplicants: 5, hiredApplicants: 1, rejectedApplicants: 2 }],
        total: 1,
        page: 1,
        limit: 10,
        totalPages: 1
      ...
      const result = await controller.findAll(1, 10, "cat-1", "PUBLISHED", "engineer");

      expect(vacancyService.findAll).toHaveBeenCalledWith(
        1, 10, "cat-1", "PUBLISHED", "engineer", undefined, undefined
      );
    ...
    it("should return paginated vacancies with default parameters", async () => {
      const mockPaginatedResponse = {
        data: [{ ...mockVacancyResponse, totalApplicants: 3, hiredApplicants: 0, rejectedApplicants: 1 }],
        total: 1,
        page: 1,
        limit: 10,
        totalPages: 1
    ...
    it("should pass date-range params to service", async () => {
      ...
      await controller.findAll(1, 10, undefined, undefined, undefined, "2024-01-01", "2024-12-31");

      expect(vacancyService.findAll).toHaveBeenCalledWith(
        1, 10, undefined, undefined, undefined, "2024-01-01", "2024-12-31"
      );
    });
  });
  ...
  describe("remove", () => {
    ...
    it("should throw NotFoundException when vacancy not found", async () => {
      vacancyService.remove.mockRejectedValue(new NotFoundException("Vacancy not found"));

      await expect(controller.remove("nonexistent-id", mockRequest)).rejects.toThrow(
        NotFoundException
      );
    });
  });
  ...
  describe("closeVacancy", () => {
    it("should close vacancy successfully", async () => {
      const closedResponse = { ...mockVacancyResponse, status: JobStatus.CLOSED };
      vacancyService.closeVacancy.mockResolvedValue(closedResponse);

      const result = await controller.closeVacancy("vacancy-1", mockRequest);

      expect(vacancyService.closeVacancy).toHaveBeenCalledWith("vacancy-1", "user-1");
      expect(result.status).toBe(JobStatus.CLOSED);
    });

    it("should throw NotFoundException when vacancy not found", async () => {
      vacancyService.closeVacancy.mockRejectedValue(new NotFoundException("Vacancy not found"));

      await expect(controller.closeVacancy("nonexistent-id", mockRequest)).rejects.toThrow(
        NotFoundException
      );
    });
  });
  ...
  describe("archiveVacancy", () => {
    it("should archive vacancy successfully", async () => {
      const archivedResponse = { ...mockVacancyResponse, status: JobStatus.ARCHIVED };
      vacancyService.archiveVacancy.mockResolvedValue(archivedResponse);

      const result = await controller.archiveVacancy("vacancy-1", mockRequest);

      expect(vacancyService.archiveVacancy).toHaveBeenCalledWith("vacancy-1", "user-1");
