...
export class ApplicationSeeder extends BaseSeeder {
  async run(): Promise<void> {
    ...
    if (applicants.length === 0) {
      console.log(
        "No applicants found. Please run applicant seeder first."
      );
      return;
    }

    // pakai pipeline hcm neuron (default) secara eksklusif
    ...
    if (!hcmPipeline) {
      console.log(
        "HCM Neuron pipeline not found. Please run recruitment pipeline seeder first."
      );
      return;
    }
    ...
    if (vacancies.length === 0) {
      console.log(
        "No vacancies found for HCM Neuron pipeline. Please run vacancy seeder first."
      );
      return;
    }
    ...
    if (stages.length === 0) {
      console.log(
        "No pipeline stages found. Please run pipeline seeder first."
      );
      return;
    }
    ...
    for (let i = 0; i < 50; i++) {
      ...
      // tentukan index stage: mayoritas di stage awal, sebagian lebih jauh
      ...
      applicationData.push({
        ...
        source: ["linkedin", "jobstreet", "website", "referral", "indeed"][
          i % 5
        ],
      ...
      // buat record StageActivity DONE untuk tiap stage yang sudah dilewati
      for (let s = 0; s < stageIndex; s++) {
        activityData.push({
          ...
          createdAt: new Date(
            Date.now() - (stageIndex - s + 1) * 3 * 24 * 60 * 60 * 1000
          ),
          updatedAt: new Date()
        });
      }

      // buat record StageActivity untuk stage saat ini
      activityData.push({
        id: this.generateId(),
        applicationId,
    ...
    console.log(
      `${applicationData.length} applications and ${activityData.length} stage activities seeded successfully`
    );
  }
}
